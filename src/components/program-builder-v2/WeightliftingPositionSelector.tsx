  ...
  return (
    <div className={cn("space-y-1", compact && "space-y-0.5")}>
        ...
        <TooltipProvider>
          <Tooltip>
            ...
            <TooltipContent className="max-w-xs">
              <p className="text-xs">
                Sélectionnez la position de départ du mouvement : du sol, en suspension, des blocks ou du rack.
              </p>
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>
      ...
      <Select value={value || "floor"} onValueChange={onChange}>
        <SelectTrigger className={cn("bg-background", compact ? "h-7 text-xs" : "h-8")}>
          ...
          {applicablePositions.map((position) => (
            <SelectItem key={position.key} value={position.key}>
              <div className="flex items-center gap-2">
                <div className={cn("w-2 h-2 rounded-full", position.color)} />
                <span>{position.labelFr}</span>
                <span className="text-muted-foreground text-[10px]">({position.labelEn})</span>
              </div>
            </SelectItem>
          ))}
        </SelectContent>
      ...
      {selectedPosition && value && value !== "floor" && (
        <p className={cn("text-muted-foreground italic", compact ? "text-[9px]" : "text-[10px]")}>
...
export const WeightliftingPositionBadge = ({
  positionKey, compact = false,
}: { positionKey?: string; compact?: boolean }) => {
  if (!positionKey || positionKey === "floor") return null;
  const position = WEIGHTLIFTING_STARTING_POSITIONS.find(p => p.key === positionKey);
  if (!position) return null;

  return (
    <Badge variant="outline" className={cn("gap-1", compact ? "text-[9px] px-1 py-0" : "text-[10px]")}>
