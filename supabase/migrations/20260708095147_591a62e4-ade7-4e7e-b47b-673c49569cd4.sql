...
-- Notify staff (club owner + staff members of the category) when an athlete responds to a session
...
DECLARE
  ...
  v_subtype text;
BEGIN
  -- Only fire when status effectively changes to present/absent
  ...
  SELECT ts.id, ts.category_id, ts.session_date, ts.session_start_time,
         COALESCE(NULLIF(ts.session_type, ''), 'Séance') AS session_type,
         COALESCE(NULLIF(ts.training_type, ''), '') AS training_type
    INTO v_session
  ...
  v_session_label := COALESCE(NULLIF(v_session.training_type, ''), v_session.session_type);
...
DROP TRIGGER IF EXISTS trg_notify_staff_on_athlete_attendance ON public.event_participants;
CREATE TRIGGER trg_notify_staff_on_athlete_attendance
  AFTER UPDATE OF attendance_status, absence_comment
  ON public.event_participants
  FOR EACH ROW
  EXECUTE FUNCTION public.notify_staff_on_athlete_attendance_response();
