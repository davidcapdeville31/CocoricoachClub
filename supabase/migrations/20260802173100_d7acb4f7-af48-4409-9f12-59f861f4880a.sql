ALTER TABLE public.club_invitations ALTER COLUMN expires_at SET DEFAULT (now() + interval '28 days');
ALTER TABLE public.category_invitations ALTER COLUMN expires_at SET DEFAULT (now() + interval '28 days');
ALTER TABLE public.athlete_invitations ALTER COLUMN expires_at SET DEFAULT (now() + interval '28 days');
ALTER TABLE public.ambassador_invitations ALTER COLUMN expires_at SET DEFAULT (now() + interval '28 days');
...
BEGIN
  ...
  v_new_expires := now() + interval '28 days';
