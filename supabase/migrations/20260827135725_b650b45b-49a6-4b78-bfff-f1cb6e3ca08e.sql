...
BEGIN
  ...
  -- Resolve readable test label (handles custom:<uuid> and custom_<slug>)
  ...
  IF NEW.test_type ~* '^custom:' THEN
    ...
    WHERE ct.id = substring(NEW.test_type from 7)::uuid;
...
-- Retro-fix existing notifications containing raw custom identifiers
UPDATE public.notifications n
SET message = regexp_replace(
  n.message,
  'custom:[0-9a-fA-F-]{36}',
  COALESCE(ct.name, 'Test personnalisé')
)
FROM public.custom_tests ct
WHERE n.notification_type = 'test_result_submitted'
  AND n.message ~ 'custom:[0-9a-fA-F-]{36}'
  AND ct.id = substring(n.message from 'custom:([0-9a-fA-F-]{36})')::uuid;