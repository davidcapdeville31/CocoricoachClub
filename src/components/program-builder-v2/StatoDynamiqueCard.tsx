...
export const StatoDynamiqueCard = ({
  config,
  exerciseName,
  onEdit,
  onRemove,
  compact = false,
  className,
}: StatoDynamiqueCardProps) => {
  const volume = calculateStatoDynamiqueVolume(config);
  
  // Format load display
  const formatLoad = () => {
    if (!config.loadValue) return null;
    const typeInfo = LOAD_TYPES[config.loadType];
    return `${config.loadValue}${typeInfo.unit}`;
  };
  
  if (compact) {
    return (
      <div className={cn(
        "flex items-center gap-2 p-2 rounded-lg bg-violet-500/10 border border-violet-500/30",
        className
      )}>
        ...
        <div className="flex-1 min-w-0">
          <div className="text-xs font-medium truncate">
            {exerciseName || 'Stato-Dynamique'}
          </div>
          <div className="text-[10px] text-muted-foreground">
            {formatStatoDynamiqueSummary(config)}
          </div>
        </div>
        {onEdit && (
          <Button 
  ...
  return (
    <div className={cn(
      "rounded-lg bg-violet-500/5 border-2 border-violet-500/30 overflow-hidden",
      className
    )}>
      {/* Header */}
      <div className="flex items-center justify-between p-3 bg-gradient-to-r from-violet-500/20 to-violet-600/20 border-b border-violet-500/20">
        <div className="flex items-center gap-2">
          <div className="p-2 rounded-lg bg-violet-500/30">
            <Zap className="h-5 w-5 text-violet-700 dark:text-violet-400" />
          </div>
          <div>
            <div className="font-semibold text-violet-700 dark:text-violet-400 flex items-center gap-2">
              ...
              <Badge variant="outline" className="text-xs border-violet-500/50">
                {SEQUENCE_TYPES[config.sequence].label}
              </Badge>
            </div>
            {exerciseName && (
        ...
        <div className="flex items-center gap-1">
          {onEdit && (
            <Button 
              variant="ghost" 
              size="icon" 
              className="h-8 w-8 hover:bg-violet-500/20"
              onClick={onEdit}
            >
      ...
      <div className="p-3 space-y-3">
        ...
        <div className="flex items-center gap-4 flex-wrap">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium">{config.sets} séries</span>
          </div>
          
          <div className="flex items-center gap-2">
            <Clock className="h-4 w-4 text-muted-foreground" />
            <span className="text-sm">{config.restSeconds}s repos</span>
          </div>
          
          {formatLoad() && (
            <div className="flex items-center gap-2">
              <Dumbbell className="h-4 w-4 text-muted-foreground" />
              <span className="text-sm font-medium">{formatLoad()}</span>
            </div>
          )}
        </div>
          ...
          <div className="p-2 rounded-md bg-amber-500/10 border border-amber-500/20">
            <div className="flex items-center gap-1 mb-1">
              ...
              {(config.staticPhases || []).length > 1 && (
                <Badge variant="outline" className="text-[10px] h-4 px-1 border-amber-500/50">
                  ×{config.staticPhases?.length}
                </Badge>
              )}
            </div>
            ...
            {(!config.staticPhases || config.staticPhases.length === 0) && config.staticDurationSeconds && (
              <div className="text-sm font-semibold">
                {config.staticDurationSeconds} sec
              </div>
            )}
          </div>
        ...
        <div className="flex items-center justify-between text-xs text-muted-foreground pt-2 border-t border-border/50">
          ...
          {volume.estimatedTonnage && (
            <span>Tonnage: ~{volume.estimatedTonnage}kg</span>
          )}
        </div>
