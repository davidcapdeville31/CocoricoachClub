DO $$ BEGIN
  BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.competition_rounds; EXCEPTION WHEN duplicate_object THEN NULL; END;
  BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.competition_round_stats; EXCEPTION WHEN duplicate_object THEN NULL; END;
  BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.athletics_records; EXCEPTION WHEN duplicate_object THEN NULL; END;
  BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.athletics_sprint_attempts; EXCEPTION WHEN duplicate_object THEN NULL; END;
  BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.athletics_throwing_attempts; EXCEPTION WHEN duplicate_object THEN NULL; END;
...
BEGIN
  ...
  ELSE
    ...
    v_has_sb := NEW.season_best IS DISTINCT FROM OLD.season_best
      ...
      AND (OLD.season_best IS NULL
        OR (NEW.lower_is_better AND NEW.season_best < OLD.season_best)
        OR (NOT NEW.lower_is_better AND NEW.season_best > OLD.season_best));
  END IF;

  IF NOT v_has_pb AND NOT v_has_sb THEN RETURN NEW; END IF;

  v_subtype := CASE WHEN v_has_pb AND v_has_sb THEN 'personal_and_season_best'
                    WHEN v_has_pb THEN 'personal_best'
                    ELSE 'season_best' END;
  v_title := CASE WHEN v_has_pb AND v_has_sb THEN '🏆 Nouveau record personnel et de saison'
                  WHEN v_has_pb THEN '🏆 Nouveau record personnel'
                  ELSE '⭐ Nouveau record de saison' END;
  v_message := CASE WHEN v_has_pb AND v_has_sb THEN format('%s a battu ses records sur %s.', v_player_name, v_event_label)
                    WHEN v_has_pb THEN format('%s a battu son record personnel sur %s.', v_player_name, v_event_label)
                    ELSE format('%s a battu son record de saison sur %s.', v_player_name, v_event_label) END;

  INSERT INTO public.notifications (
    user_id, category_id, notification_type, notification_subtype,
    title, message, priority, metadata
  ) VALUES (
    v_user_id, NEW.category_id, 'athlete_record', v_subtype,
    v_title, v_message, 'high',
    jsonb_build_object(
