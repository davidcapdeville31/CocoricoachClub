...
export const ClusterCard = ({
  ...
  const getLoadDisplay = () => {
    ...
    switch (config.loadType) {
      case 'percentage':
        return `${config.loadValue}% 1RM`;
      case 'weight_kg':
        return `${config.loadValue}kg`;
      case 'rpe':
        return `RPE ${config.loadValue}`;
      default:
        return null;
    }
  };
  ...
  return (
    <Card className={cn(
      "border-l-4 border-l-orange-500 bg-orange-500/5 dark:bg-orange-500/10 overflow-hidden",
      className
    )}>
      {/* Header */}
      <div 
          ...
          <div className="grid grid-cols-4 gap-2 text-center">
            ...
            <div className="p-2 rounded-md bg-background border">
              <p className="text-lg font-bold text-orange-600">
                {volume.repsPerSet === 'variable' ? '~' : volume.repsPerSet}
              </p>
              <p className="text-[10px] text-muted-foreground uppercase">Reps/série</p>
            </div>
            <div className="p-2 rounded-md bg-background border">
              <p className="text-lg font-bold text-orange-600">
                {volume.totalReps === 'variable' ? '~' : volume.totalReps}
              </p>
